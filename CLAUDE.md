...
本仓库是一个 **Claude Code 工程**，Claude Code 当创作驾驶舱；Codex、Claude Code 等仅是工程开发工具，不是运行期业务 Provider。运行期模型路由只由 `config/model_routes.yaml` 的显式位点决定，禁止 fallback、自动降级或隐式 Provider 切换。
