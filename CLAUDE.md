...
## 这是什么
一个 **Claude Code 工程**(不是独立程序):写死的 Python 脚本干确定性脏活,Claude Code 当创作驾驶舱。目标 = 一个"越用越好"的抖音内容创作系统(自进化 agent 的**实质**,载体是程序)。

**Codex/Claude Code 只是当前用来编写这个仓库代码的工程工具,不是系统运行期的业务 provider**(2026-07-09 新增,防止再次把"用什么工具写代码"和"系统运行时调用什么模型"混为一谈)。系统运行期统一使用 Mimo,通过 `config/model_routes.yaml` 这一个显性配置入口生效,里面点名三个模型位点——`dialogue_model`(日常对话)/`business_model`(业务分析类 Skill 节点)/`writing_model`(创作生成类 Skill 节点),当前三者都绑定同一个 Mimo provider,但各自可以独立改指向不同 provider,不受"用什么工具开发"影响。未来若要切到 GPT/其他 API,必须显式改这个配置文件里对应位点的 `provider_ref`,不允许因为开发工具换了就隐式带动业务链路换模型,禁止任何 fallback/自动降级(见 `BUSINESS_MODEL_SWITCH_TO_GPT_PLAN.md`)。

飞书/Obsidian 目前只是 `scripts/core/host/` 预留的适配器位置(`DEFAULT_ALLOWED_HOSTS` 里的 `feishu`),**不是现状**——飞书实时集成在一次 legacy removal 里被整体删掉、还没重建;Obsidian 在当前代码里几乎没有真实写入。不要假设这两者已经在跑。
...
- **模型路由 per-node,唯一显性入口**:`config/model_routes.yaml` 是系统运行期模型路由的唯一配置文件,点名三个位点——`dialogue_model`/`business_model`/`writing_model`,当前全部 = Mimo(经 Hermes)。**写代码用什么工具(Codex/Claude Code)与这三个位点绑定什么模型完全无关**,不允许因为开发工具是 Codex/Claude Code 就把某个业务位点默认绑定成"走 Codex/Claude 对话"。可替换 provider(以后要接 GPT/其他 API 再改对应位点的 `provider_ref`),**禁止 fallback、禁止自动降级、禁止隐式 provider 切换**——同一位点某次调用失败就是失败,不会静默换一个 provider 重试。
...
- **生产 Host 边界(已归档)**:`scripts/core/host/`(`production_host.py`)曾设计为平台中立入口,但全仓库引用搜索证实它从未被任何真实生产入口调用,只被自己家族的 `verify_goal_*.py` 自证脚本使用——2026-07-09 已归档到 `archive/dead_goal_chain_20260709/`,`scripts/core/` 下目前**没有**这一层。以后若真的需要"外部平台先变成 Host message 再经白名单"这套边界,是一次新的架构决策,不要假设它已经存在或可以直接复用归档代码。
...
- **原子 Skill 层**:`runtime_skills/`——按 CR-003A(可移植原子 Skill:自带输入输出 schema、不读数据库、不串联其他 Skill)设计的独立 Skill 包,目前有 12 个业务 Skill(`sample_deep_analyze`/`tactic_extract`/`source_to_topic`/`script_generate`/`script_review`/`content_plan`/`content_classify`/`content_relation_judge`/`research_evidence_extract`/`production_research_plan`/`experiment_review`/`experience_revision_propose`)+ 1 个测试探针(`runtime_probe`,不对应真实业务能力,不要算进"13个业务Skill")。**Binding 现状**(2026-07-10 更新):`sample_deep_analyze`/`source_to_topic`/`content_plan`/`script_generate` 四个已接上真实数据,串成"选题→大纲→成稿"主链路且有端到端集成测试(`tests/core/test_topic_to_script_chain_integration.py`),中途卡在人工审核闸门(`scripts/core/experience/review_queue.py`)上;其余 8 个仍只能吃 `fixtures.yaml` 假样例过契约测试。**选题这一环目前只是简化版**(只用"对标爆款+评论区"两种料源,"研究缺口/当下热点"两种候选来源还没接)。**2026-07-11 澄清:当前设计没有冻结"选题打分制"**——不接受把旧系统的评分维度/打分表/score-rank机制当作"缺口"补回;`.claude/skills/归纳`(旧系统产出"选题判断维度"评分标准的技能)已删除且**不予恢复**,不是"待补位"。当前真正的缺口是:当前设计里的选题判断规则、领域约束、去重/冷却、候选来源、人工确认门槛,是否已经被真实代码落地并验证——详见 `ROADMAP.md`,不要臆测选题环节已经做对。**创作链路当前状态只到"状态机已验证"这一档**:端到端集成测试证明的是外键链路+人工审核闸门真的会拦人,全部 12 个 Skill 的测试底层都是各自 `make_*_harness()` 默认的确定性假 Provider(关键词字符串匹配)。**2026-07-11 更正**:`sample_deep_analyze` 曾在 2026-07-08 真实调用过一次 Hermes(`business_model` 位点),但产出从没被人看过、没判断过好不好用;`source_to_topic`/`content_plan`/`script_generate`(含 `writing_model` 位点)则真的一次都没被真实调用过。不得在任何地方宣称"创作功能已完成"或"内容生成已验证"——只能说"状态机已验证,真实大模型生成未验证"。
...
- **原本两套平行架构,已清场归档"死链"部分**(2026-07-10 发现,2026-07-09 用户拍板清场重构后处理):`scripts/core/{persistence,scheduler,production,correction,workflow,host,hermes,state,research}` 曾是一条完整但从未接过真实数据的"正式生产链路"(Goal01-12,`PersistenceStore`/`VersionRef`/`goal10_corrections` 修正传播框架都在这套里,只在各自的 `verify_goal_*.py` 自证脚本里跑通过)。**逐一用全仓库引用搜索(而非文件名)核实后**:`correction`/`production`/`host`/`hermes`/`state` 整个目录、`workflow/goal_phase5_business_workflow.py`,确认零真实生产入口依赖,已归档到 `archive/dead_goal_chain_20260709/`(连同只测试它们的 4 个 pytest 文件)。`persistence`(`content_hash`/`PersistenceStore`)、`scheduler`(`Goal03Scheduler`)、`workflow/goal05_workflow.py`、`research/goal06_formal_research.py` 出乎意料地**保留在原地**——它们被 `scripts/core/model_gateway/formal_skill_adapter.py` 的 `make_*_harness()` 或 `scripts/core/external_adapters/goal_phase4_external_adapters.py` 真实传递依赖,而后者又被 `run_competitor_registration_full.py`/`run_reverse_prep.py` 真实导入。**结论**:判断一个模块是否该归档,只能用"从真实生产入口出发算传递闭包"这个机械方法验证,不能凭文件名/所在目录猜(这两个模块的名字里都带着 `goal0X`,看起来像旧链路,实际是共享基础设施)。`scripts/core/runtime/` 和 `scripts/core/external_adapters/goal_phase4_external_adapters.py` 本身未在本次归档范围内(未被列入待清理清单),现状见 `HANDOFF_STATE.md`。
