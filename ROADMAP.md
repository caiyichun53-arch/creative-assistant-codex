...
#### 真实选题流程还缺什么(2026-07-10 用户指出,并逐条核实过,不是猜的)
...
- ❌ 打分排序(判 + 选)——**2026-07-10 已核实:"选题判断维度.md"这份文件全仓库/全vault路径搜索都不存在**,而且原本负责产出它的技能(`.claude/skills/归纳`)已经在 2026-07-08 被删除,当时以为 `runtime_skills/tactic_extract` 能顶替,但核实后发现 `tactic_extract` 产出的是 `common_patterns`/`example_candidates`(写作打法),跟"评分维度"是两回事,顶替不了。这把"尺子"目前完全不存在,不是"还没做",是"生产它的技能已经被删掉、没人补位"。
- ❌ 人工终审——`review_queue.py` 现在只有"通过/不通过"二选一,而且**没有对接项目里已经存在的、更合适的"修正+留痕"框架**(`scripts/core/correction/goal10_corrections.py`,见下方"人工审核")。

**建议节奏**:这几项工作量都不小,且②③依赖①(打分排序要先有评分标准),不建议在治理修复的同一批里赶工。下一次专门做选题流程,建议顺序:研究缺口(复用已有的 research_evidence_extract 骨架)→ 重建"归纳"这一环产出选题判断维度(需要重新设计,不是简单复用 tactic_extract)→ 热点采集(需要用户先定数据源/工具)→ 多候选打分排序 → 终审形态重新设计(见下方 goal10_corrections 集成问题)。
