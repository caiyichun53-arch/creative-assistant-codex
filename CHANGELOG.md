# CHANGELOG(详细历史记录,只增不改)

> **2026-07-09 拆分说明**:这份文件是 `HANDOFF_STATE.md` 在 2026-07-09 之前积累的全部详细过程记录(逐次会话的事故复盘、真实数据验证过程、决策推理链路)——2026-07-09 的外部工程审计发现原 `HANDOFF_STATE.md` 已经涨到 239 行自由文本流水账,新接手的人很难在合理时间内扫描完,也容易漏看关键信息。拆分之后:**`HANDOFF_STATE.md` 只保留"现在是什么状态"的结构化快照**(模块状态表、阻塞项、待用户决定事项),**这份 `CHANGELOG.md` 保留"过去发生过什么"的完整叙事**,新会话不需要逐字读完这份文件才能开工——需要查某个具体决策的来龙去脉(比如"为什么 excess_threshold 是2.0不是3.0")时再来这里搜,不是每次开工的必读项。以下是拆分前的原始内容,原样保留,之后的新记录追加在文件末尾,不改写已有条目。

---

# HANDOFF_STATE(拆分前的历史内容,原样保留)
