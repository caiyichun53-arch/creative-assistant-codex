...
[已完成](部分) 2026-07-08 首次接线"单条深度分析";2026-07-09 接上第二步"选题生成"、第三步"钩子+大纲规划"。"归纳共性"和"自营P基线怎么用"这两块还没做,`script_generate`(大纲→成稿)是链路最后一环,还没接。

**这一层干什么**：把已经备好料(有转写文字稿+评论)的爆款,喂给大模型做"深度分析",提炼出可复用的选题手法/开头手法/结构手法——旧系统管这叫"DNA拆解",现在不这么叫了,对应的是 `runtime_skills/sample_deep_analyze` 这个独立的原子能力。分析结果再喂给 `runtime_skills/source_to_topic` 生成候选选题,候选选题再喂给 `runtime_skills/content_plan` 规划钩子(开头)和大纲——这是"选题→大纲→成稿"这条创作主链路的前三环。
...
- `script_generate`(大纲→成稿草稿)——这条链路的最后一环,还没接。
- "归纳共性"(`tactic_extract`,把好几条分析结果归并成一条通用打法)还没做——它至少需要2条同类的分析结果才能跑,排在选题→大纲→成稿主链路之后。真接上之后,`content_plan` 里"复用同一条爆款自己手法"这个替代方案就可以换成真实的 `tactic_extract` 输出。
