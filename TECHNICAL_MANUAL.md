...
**字符/token 上限已彻底取消(2026-07-13,用户明确拍板)**：这条主链路(以及 `tactic_extract`)之前每一步都有三层长度护栏——①`*_BUSINESS_CONTRACT.yaml` 里声明的 `context_budget.max_input_tokens`/`token_budget.max_output_tokens`(token预算)和 `input_length_limits`(每个字段的字符数上限);②`formal_skill_adapter.py` 运行时对 `maxLength` 的真实校验(超过就报 `FormalSkillValidationError`);③每个绑定脚本自己手写的 `_MAX_CHARS` 常量,在拼输入前就先切一刀。核查后发现:①从来只是"声明存在、数字大于0"这一层被检查过,从没真的数过一次真实要发的内容有多少 token,是"看起来管了、其实没管"的假护栏;③里至少一处(`sample_deep_analyze` 的转写文字稿截断)真实证据证明截断本身会造成事故——2026-07-09 曾经因为2200字的截断,让模型对着被腰斩的转写编造了一段从没出现过的结尾。用户看完这个情况后的决定是**彻底不设任何上限**(不是"调高上限"或"改成报错代替截断"这种折中方案,是明确拒绝了折中,选择完全不设边界):三层护栏全部拆除,`config/model_routes.yaml` 之外不再有任何字符数/token数的裁剪或拒绝逻辑,真实内容原样完整发给模型。**这是用户明确接受的风险,不是疏漏**:真实调用现在有可能因为内容过长被 Hermes 自己的 API 拒绝、或者让单次调用的成本变高,系统不会再提前拦截或警告——这类问题只能在真的发生真实调用时才会被发现。改动范围:全部12份 `*_BUSINESS_CONTRACT.yaml` 契约文件 + `runtime_skills/*/input_schema.yaml`/`output_schema.yaml` 的对应副本 + `formal_skill_adapter.py`/`business_route_registry.py` 的运行时校验 + 5个已经真实调用过模型的绑定脚本(`run_sample_deep_analyze.py`/`run_source_to_topic.py`/`run_content_plan.py`/`run_script_generate.py`/`run_script_review.py`/`run_tactic_extract.py`)里全部的截断常量和 `max_completion_tokens` 参数。

**怎么把一条真实爆款变成一份成稿草稿**：
1. `python -m scripts.core.experience.run_sample_deep_analyze --limit N`——挑一条已经完成备料(转写完成)、但还没做过分析的爆款,只把大模型"分析"这一步真正需要的东西喂给它:完整转写文字稿(2026-07-13 起不再截断,见下方"字符/token 上限已彻底取消"小节)、点赞/评论/收藏/转发数、领域标签——**不会把数据库内部的字段名、内部编号这些东西传进去**。拿到结果(选题手法/开头手法/结构手法三段话)后存进 `hit_deep_analysis` 表,带版本号,不覆盖旧分析。
2. `python -m scripts.core.experience.run_source_to_topic --limit N`——挑一条已经分析过、但还没生成过候选选题的分析结果,把"选题手法/开头手法/结构手法"三段话 + 真实热门评论 + 这个领域最近30天登记的真实热点(**2026-07-13,D1,BR-TOPIC-006**:`hotspot_events` 自动纳入,不需要人工逐条关联)当证据喂给 `source_to_topic`,生成一条候选选题(标题+切入角度+支撑证据)。**领域约束(2026-07-13,D2,BR-TOPIC-001)**:候选所属账号的领域标签如果不在已知领域枚举内,自动强制标成"待复核",不能悄悄流到下一步——只做这一条确定性检查,不做"内容是否真的符合该领域调性"这类需要模型语义判断的检查(避免变相打分)。**去重/冷却(2026-07-13,D3,BR-TOPIC-001)**:生成后紧接着用 `content_relation_judge`(这个 Skill 契约早就写好、但全仓库从没有真实绑定脚本调用过,这次是第一次真正接上真实数据)跟同一账号最近30天内、最多20条候选逐条比对,命中"实质重复"(同一事物/表述等价/互相包含,4种关系类型之一)就标"待复核"并记下跟哪条候选疑似重复——不自动拒绝,人工审核时会看到。30天窗口、20条比较上限、"关联判断本身"(`relation_summary` 这个输入字段,描述生成前是否已经做过关系检查)仍然是老实的占位文字(这一步的去重发生在生成**之后**,不是生成前,所以这个占位描述依然准确,不是过时没改)。结果存进 `topic_candidates` 表,同样带版本号。
...
- **选题候选来源现状(2026-07-13 更新)**:对标爆款/评论区/当下热点三种真实来源已接(见上文第2步),**研究缺口(原定第四种)明确没接**——这个概念本身在仓库里找不到一份能确认是当前设计权威的定义(交互式协作创作技能里的旧写法自称过时;另一个编程工具的会话技能定义里完全没有这个词),不猜、不编造,留到能重新核对原总控文档时再定义,不是排期问题。**选题判断规则、去重/冷却、领域约束**这三项(2026-07-13,D1/D2/D3)已经落地并验证,不再是空白——具体规则见上文第2步、`BUSINESS_RULE_CATALOG.yaml` 的 `BR-TOPIC-001`。**2026-07-11 澄清仍然成立**:当前设计没有冻结"选题打分制",`.claude/skills/选题/SKILL.md` 里描述的"选题判断维度"评分排序是该交互式技能自己的旧方法,不代表 `runtime_skills/source_to_topic` 当前设计要对齐的目标,不得当作缺口去补;这次 D2 落地的领域约束、D3 落地的去重/冷却,用的都是确定性规则/关系类型匹配,不是评分排序。另外,搜到的外部视频(A3 领域话题标签搜索)怎么变成正式选题这一环仍然没接(见下方"领域话题标签搜索"小节)。
