...
- "归纳共性"(`tactic_extract`,把好几条分析结果归并成一条通用打法)还没做——它至少需要2条同类的分析结果才能跑,排在选题→大纲→成稿主链路之后。
- `content_plan`(选题→钩子+大纲)、`script_generate`(大纲→成稿)——这条链路的下两环,还没接。
