...
- **原子 Skill 层**:`runtime_skills/`——按 CR-003A(可移植原子 Skill:自带输入输出 schema、不读数据库、不串联其他 Skill)设计的独立 Skill 包,目前有 12 个业务 Skill(`sample_deep_analyze`/`tactic_extract`/`source_to_topic`/`script_generate`/`script_review`/`content_plan`/`content_classify`/`content_relation_judge`/`research_evidence_extract`/`production_research_plan`/`experiment_review`/`experience_revision_propose`)+ 1 个测试探针(`runtime_probe`,不对应真实业务能力,不要算进"13个业务Skill")。**Binding 现状**(2026-07-09 审计核实,不再是"一个都没接"):只有 `sample_deep_analyze` 接上了真实数据(`scripts/core/experience/run_sample_deep_analyze.py`,真实数据组装→调用 Skill→写回 `hit_deep_analysis` 表,完整闭环);其余 11 个仍只能吃 `fixtures.yaml` 假样例过契约测试,拿不到真实数据产出。接下来按什么顺序接、接到哪个,见 `ROADMAP.md`,不要臆测某个 Skill "已经能直接跑真实数据"。
