...
## 阶段4项目路径迁移残留分类与收口（2026-08-29）

阶段4将 Creation Assistant 的唯一正式项目路径迁移为 `I:/Creation-Assistant`。Git 历史、当前分支和 HEAD 保持不变；正式 runtime 继续使用外置目录 `I:/Creation_assistant-runtime`，正式数据库未迁移、未复制为新的活动库，也未修改正式业务数据。

### 当前入口核对

- Windows 平台启动入口的工作目录已经指向 `I:/Creation-Assistant`。该入口记录的外部解释器目标目前不存在，因此启动动作本身当前不可执行；它不再指向旧项目，也不能形成旧项目的业务控制权。这是外部启动环境残留，不在本步改动。
- 旧 Windows 监听器残留位于用户 Startup 目录，仍指向不存在的旧监听脚本路径；脚本目标不存在，因此不能启动 Creation Assistant 正式业务。当前正式启动不依赖它，归类为历史系统残留，无当前控制权。
- WSL、Core、MCP、调度器和 Hermes 的当前有效项目路径均指向新项目。项目内当前有效配置和启动入口不再引用旧项目路径；迁移快照和历史记录中的旧路径只用于保存证据。
- 用户级 Creation Assistant 自动化记录仍保存旧工作目录，但状态为 `PAUSED`，当前不会触发正式业务；本步不修改用户级 Codex 配置。
- Stop hook 的实际失败原因是当前受限运行环境拒绝新项目下 hook 事件记录的写入，不是旧项目路径依赖。当前项目 hook 配置已经指向新项目；本步不为适配外部权限而修改 hook 行为。

### 旧目录残留

- 旧目录不再包含 Git 仓库或可运行的 Creation Assistant 项目本体。
- 旧目录中保留的 16 份历史/异常资料内容未改动、未纳入提交。
- 旧目录中的 5 个测试临时目录没有 Git 仓库，没有当前配置、代码或 runtime 引用，也不会被当前项目自动发现；它们是不可删除的历史测试临时残留，不属于当前项目，未修改权限或 ACL。

### 阶段4结论

按“当前运行控制权”判断，旧项目路径、失效监听器和 5 个临时目录均不再控制当前 Creation Assistant 业务。阶段4项目路径迁移条件满足并正式关闭；外部 Windows 启动入口的失效解释器目标和 Stop hook 的受限环境写入失败分别保留为外部运行环境问题，不改变项目路径归属，也不进入阶段5处理。

本步仅记录迁移事实并封存路径修改；未执行正式 cold-start、reset、resume 或 daily，未调用真实模型，未修改正式数据库，16 份历史/异常资料保持原样。
