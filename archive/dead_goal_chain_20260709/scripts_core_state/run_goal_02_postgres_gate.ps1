...
$verifyPath = Join-Path $PSScriptRoot "verify_goal_02_postgres.sql"
...
foreach ($path in @($goal01SchemaPath, $goal02SchemaPath, $verifyPath)) {
...
if ($DryRun) {
    Write-Output "GOAL-02 PostgreSQL gate dry run"
...
Write-Output "GOAL-02 PostgreSQL gate started"
& $PsqlPath @arguments
$exitCode = $LASTEXITCODE
if ($exitCode -ne 0) {
    [Console]::Error.WriteLine("GOAL-02 PostgreSQL gate failed with exit code $exitCode")
    exit $exitCode
}

Write-Output "GOAL-02 PostgreSQL gate passed"
