...
class HermesCoreBridge:
    ...
    def dispatch(self, message: HermesInboundMessage) -> HermesDispatchResult:
        ...
        if existing is not None:
            return existing
        core_result = self.core.execute(message.core_envelope())
        correlation_id = message.core_envelope().correlation_id or message.idempotency_key
        with self.core.conn:
            ...
            response_outbox_id = self.core.store.enqueue_outbox(
                ...
                payload=_response_payload(message, core_result.status, core_result.state, core_result.reason),
                correlation_id=correlation_id,
    ...
    def _existing_host_result(self, message: HermesInboundMessage) -> HermesDispatchResult | None:
...
class FeishuResponseDispatcher:
    ...
    def dispatch(self, scheduler_payload: dict[str, Any]) -> FeishuSendResult:
        ...
        if topic != GOAL11_RESPONSE_TOPIC:
            raise HermesHostBindingError("unsupported response topic")
        payload = scheduler_payload.get("payload") or {}
        if not isinstance(payload, dict):
            raise HermesHostBindingError("response payload must be an object")
        ...
        if outbox_id in self.fail_once_outbox_ids and outbox_id not in self.failed_outbox_ids:
            ...
            raise HermesHostBindingError("injected fake Feishu send failure")
        with self.conn:
...
def _response_payload(
    ...
    correlation_id = message.core_envelope().correlation_id
    return {
        "binding": "feishu",
        "reply_channel_id": message.reply_channel_id,
        "source_event_id": message.source_event_id,
