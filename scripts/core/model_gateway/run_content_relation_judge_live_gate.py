...
STATUS_PATH = ROOT / "CONTENT_RELATION_JUDGE_LIVE_GATE_STATUS.yaml"
REPORT_PATH = ROOT / "CONTENT_RELATION_JUDGE_LIVE_GATE_REPORT.md"


@dataclass
class CountingContentRelationJudgeLivePort:
...
def build_live_route(config: LiveGateConfig, contract: FormalSkillContract) -> ModelRoute:
    ...
    return ModelRoute(
        ...
        config_hash=content_hash(
            ...
            "content_relation_judge.live_route.v1",
        ),
        parameters={
            "temperature": 0,
...
def build_live_port(config: LiveGateConfig) -> CountingContentRelationJudgeLivePort:
    provider_settings = hermes_subscription_provider_settings(config)
    adapter = HermesModelProviderAdapter(
    ...
    return CountingContentRelationJudgeLivePort(adapter=adapter)
...
def run_gate(config: LiveGateConfig, *, environment: str = "validation") -> dict[str, Any]:
    ...
    harness = make_content_relation_judge_harness(route=route, provider=live_port)
    try:
        payload = sample_content_relation_judge_input(
            request_id="content-relation-judge-live-01",
        ...
        if step.status != "succeeded" or result is None:
            raise RuntimeError(f"live content_relation_judge gate did not materialize a result: {step}")
        validate_content_relation_judge_output_semantics(payload, result["output"])
        ...
        if envelope_row is None:
            raise RuntimeError("missing content_relation_judge model run envelope")
        ...
        status = {
            ...
            "relation_direction": result["output"]["relation_direction"],
        ...
        if status["dry_run_fallback"] or status["fake_port_fallback"]:
            raise RuntimeError("live content_relation_judge gate unexpectedly used fallback")
        if len(outbox) != 1:
            raise RuntimeError("live content_relation_judge gate expected exactly one success outbox event")
        return status
    finally:
        harness.close()
...
def write_report(status: dict[str, Any], path: Path = REPORT_PATH) -> None:
    lines = [
        "# CONTENT_RELATION_JUDGE Live Provider Gate",
        "",
        ...
        f"- relation_type: `{status['relation_type']}`",
        f"- relation_direction: `{status['relation_direction']}`",
