param(
    ...
    [string]$DatabaseName = "goal_business_skill_content_classify_01",
    [string]$EvidencePath = "",
...
$verifyPath = Join-Path $PSScriptRoot "verify_content_classify_postgres.sql"
if (-not $EvidencePath) {
    $EvidencePath = Join-Path $root "validation_evidence\GOAL-BUSINESS-SKILL-CONTENT-CLASSIFY-01_POSTGRES.md"
}
if (-not $ContainerName) {
    $suffix = [Guid]::NewGuid().ToString("N").Substring(0, 8)
    $ContainerName = "content-classify-pg-$suffix"
}
...
INSERT INTO command_receipt(
    ...
    '018f0000-0000-7000-8000-00000000f001',
    'content_classify.postgres.concurrent',
    'content-classify-concurrent-claim',
    'request-hash',
    '{"accepted": true}'::jsonb,
...
INSERT INTO scheduler_job(
    ...
    '018f0000-0000-7000-8000-00000000f002',
    'formal_skill.execute',
    'queued',
    0,
    now(),
    3,
    '{"formal_skill_id":"content_classify","input":{"request_id":"pg-concurrent"}}'::jsonb,
    '018f0000-0000-7000-8000-00000000f099',
    '018f0000-0000-7000-8000-00000000f001'
);
WITH picked AS (
    SELECT job_id
      FROM scheduler_job
     WHERE job_id='018f0000-0000-7000-8000-00000000f002'
       AND status='queued'
...
updated AS (
    UPDATE scheduler_job j
       SET status='leased',
           ...
           current_attempt_id='018f0000-0000-7000-8000-00000000f003',
...
"@
    ...
    $evidence = @(
        "# GOAL-BUSINESS-SKILL-CONTENT-CLASSIFY-01 PostgreSQL Evidence",
        "",
        ...
        "- content_classify_sql_gate: $verifyPath",
...
Write-Output "GOAL-BUSINESS-SKILL-CONTENT-CLASSIFY-01 PostgreSQL gate passed"
