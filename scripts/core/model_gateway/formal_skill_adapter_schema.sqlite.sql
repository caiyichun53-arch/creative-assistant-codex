...
CREATE TABLE IF NOT EXISTS formal_business_skill_result_index (
    ...
    skill_run_id      TEXT NOT NULL REFERENCES formal_business_skill_run(skill_run_id) ON DELETE RESTRICT,
    skill_version     TEXT NOT NULL,
    input_hash        TEXT NOT NULL,
...
CREATE TRIGGER IF NOT EXISTS immutable_formal_business_skill_run_update
BEFORE UPDATE ON formal_business_skill_run
BEGIN
    SELECT RAISE(ABORT, 'formal_business_skill_run is immutable');
END;

CREATE TRIGGER IF NOT EXISTS immutable_formal_business_skill_run_delete
BEFORE DELETE ON formal_business_skill_run
BEGIN
    SELECT RAISE(ABORT, 'formal_business_skill_run is immutable');
END;

CREATE TRIGGER IF NOT EXISTS immutable_formal_business_skill_result_index_update
BEFORE UPDATE ON formal_business_skill_result_index
BEGIN
    SELECT RAISE(ABORT, 'formal_business_skill_result_index is immutable');
END;

CREATE TRIGGER IF NOT EXISTS immutable_formal_business_skill_result_index_delete
BEFORE DELETE ON formal_business_skill_result_index
BEGIN
    SELECT RAISE(ABORT, 'formal_business_skill_result_index is immutable');
END;
